...
# fig 3 supp fig 2 e: go enrichment of all DAGs
real_fig_3_supp_2e <- ggplot(gcAll_revi %>%
       mutate(Description = fct_reorder2(Description,
         ...
         slice(1:10), aes(x = GeneRatio,
                      ...
                      size = Count,
                      color = p.adjust)) +
  labs(x = bquote(Gene~Ratio),
       color = bquote(q-value),
       size = bquote(Count)) +
  geom_point() + theme_classic(base_size = 4, base_family = "sans") +
  ...
  theme(legend.position = c(0.775, 0.7),
        ...
        axis.text.x = element_text(color = "black"),
          axis.text.y = element_text(color = "black"),
          axis.ticks = element_line(color = "black")); real_fig_3_supp_2e
ggsave("real_fig_3_supp_2e.pdf", plot = real_fig_3_supp_2e, units = "in",
       height = 1.75, width = 1.75) # 1 inch = 72 pt
...
# supplementary table 1: gc and soma deseq2 results
real_table_supp_1 <- het_excludeShrink_plot %>%
                   full_join(somata_hetShrink_plot %>%
                               select(ensembl_id, baseMean, log2FoldChange, 
                                      lfcSE, pvalue, padj, significant,
                                      direction), 
                             by = "ensembl_id") %>%
  filter(ensembl_id != "ENSMUSG00000117786") %>%
  select(-label, -contains("number_"), -contains("sfari"), -contains("id_"))
colnames(real_table_supp_1) <- sub("\\.x", "\\.gc", 
                                   colnames(real_table_supp_1))
colnames(real_table_supp_1) <- sub("\\.y", "\\.soma", 
                                   colnames(real_table_supp_1))
write.csv(real_table_supp_1, file = "real_table_supp_1.csv",
          row.names = F)
